...
export default function Chat() {
  ...
  const [ typeCounter, setTypeCounter ] = useState<TypeCounter>({
    introversion: 0, extraversion: 0, judging: 0, perceiving: 0,
    sensing: 0, intuition: 0, thinking: 0, feeling: 0
  });
  ...
  const classifyChoices = async () => {
    try {
      ...
      const classResult: Array<string> = (firstChoice) ? results['choice_1'] : results['choice_2'];
  ...
  function PersonalityTest(): JSX.Element  {
    if (number == 0) {
      return (
        <form id="insertWord" onSubmit={(event) => {event.preventDefault(); onClickNext()}} className="flex flex-col">
          <input
            form="insertWord"
          ...
          />
          <button form="insertWord" type="submit" className="mt-8 text-white bg-gradient-to-br from-green-400 to-blue-600 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-green-200 dark:focus:ring-green-800 font-medium rounded-lg text-sm py-2.5 w-20" >
            Next
          </button>
        </form>
      );
      ...
      return (
        <div>
          ...
          {question.hasOwnProperty("choices") ? ((question.choices !==  undefined && question.background.length > 0) ? question.choices.map((choice, index) => (
            <button key={index} type="button" onClick={onChoice} className="text-left text-white bg-gradient-to-br from-green-400 to-blue-600 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-green-200 dark:focus:ring-green-800 font-medium rounded-lg text-sm px-5 py-2.5 me-2 my-2">
              {choice}
            </button>
          )) : "") : ""}
        </div>
  ...
  return (
    <div className="flex flex-col w-full max-w-md py-24 mx-auto stretch">
      ...
      {/* <button onClick={() => {if (questionReady) onClickNext()}} className="mt-8 text-white bg-gradient-to-br from-green-400 to-blue-600 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-green-200 dark:focus:ring-green-800 font-medium rounded-lg text-sm py-2.5 w-20" >
        Next
      </button> */}

      {/* {questionReady ? <button onClick={onClickNext} className="mt-8 text-white bg-gradient-to-br from-green-400 to-blue-600 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-green-200 dark:focus:ring-green-800 font-medium rounded-lg text-sm py-2.5 w-20" >
        Next
      </button> : null} */}
    </div>
  );
