...
export async function POST(req: Request) {
  ...
  const default_prompt = [
    { role: "system", content: ["You are a writer for an interactive story. You need the stories to be brief to capture the attention of your readers. You will start each adventure with",
    ...
    { role: "user", content: ["Continue the interactive story with the choice:\n look for the route map,",
        ...
        "of the format: { background: \"Write the background in 100 characters\", choices: [\"Write choice 1 in 100 characters.\",",
        "\"Write choice 1 in 100 characters.\"] }"].join(" ") },
    { role: "assitant", content: ["{ background: \"Soon after, you see a mountain, with a number of villages scattered across the",
    ...
    { role: "user", content: ["Continue the interactive story with the choice:\n Phew... thank God it's",
        ...
        "of the format: { background: \"Write the background in 100 characters\", choices: [\"Write choice 1 in 100 characters.\",",
        "\"Write choice 1 in 100 characters.\"] }"].join(" ") },
    { role: "assitant", content: ["{ background: \"You walk into the cave and see a giant bird. It looks upset and it's spreading",
  ...
  const size = (messages.length < 2) ? 0 : messages.length - 5
  ...
  const response = await openai.chat.completions.create({
    ...
    stream: true,
    max_tokens: 300,
    messages: [...default_prompt, ...(messages.splice(size, messages.length))],
  });
